...
void init() {
	...
	eeprom_update_byte((uint8_t*)TAMAS, var_8bit);
	eeprom_update_word((uint16_t*)LEVI, var_16bit);
	eeprom_update_dword((uint32_t*)BALINT, var_32bit);
	eeprom_update_float((float*)CSABA, var_float);
	eeprom_update_block((const void*)array,(void*)ROBI, 6);
#endif

#ifdef READ_FROM_EEPROM
    // TODO: read the values from the EEPROM to the variables
	var_8bit = eeprom_read_byte((uint8_t*)TAMAS);
	var_16bit = eeprom_read_word((uint16_t*)LEVI);
	var_32bit = eeprom_read_dword((uint32_t*)BALINT);
	var_float = eeprom_read_float((float*)CSABA);
	eeprom_read_block((void*)array, (const void*)ROBI,6);
#endif
}
